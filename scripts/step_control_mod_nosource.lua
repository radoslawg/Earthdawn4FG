...
function onInit()
	...
	modifierWidget.setPosition("bottomright", 5, 0);
	modifierWidget.setVisible(false);
...
function onDrag(button,x,y,dragdata)
	if getValue() >= 1 then
		...
		dragdata.setDescription(description[1] .. " (Step " .. getValue() .. ")");
...
function onDoubleClick()
	if getValue() >= 1 then
		...
		mod = 0;
		dice, mod = Step.getStepDice(getValue());
		ChatManager.control.throwDice("dice",dice, mod, description[1] .. " (Step " .. getValue() .. ")");
	end
end
