...
function onDrag(button,x,y,dragdata)
	if getValue() >= 1 then
		...
		dragdata.setDescription(window[field[1]].getValue() .. " " .. description[1] .. " (Step " .. getValue() .. ")" );
...
function onDoubleClick()
	if getValue() >= 1 then
		...
		mod = 0;
		dice, mod = Step.getStepDice(getValue());
		ChatManager.control.throwDice("dice",dice, mod, window[field[1]].getValue() .. " " .. description[1] .. " (Step " .. getValue() .. ")");
	end
end
